...
const Row: React.FC<HistoryRowProps> = ({ interview, index }) => {
  ...
  return (
    <tr
      className={`${getColor(index)}  cursor-pointer  hover:bg-black-200 text-sm  text-white lg:leading-tight md:leading-tight }`}
      onClick={() => clickHandler(interview.objectId)}
      cursor-pointer
    >
      <td className="px-4 py-2 text-left rounded-tl-2xl rounded-bl-2xl">
        {index + 1}
      </td>
      ...
      <td className="px-4 py-2 text-left">
        ...
        <div className="lg:text-[10px] md:text-[8px] hidden md:table-cell">
          {time}
        </div>
      </td>
      <td
        className={`px-4 py-2 hidden md:table-cell ${getDifficultyColor(interview.difficultyLevel)}`}
      >
      ...
      <td className="px-4 py-2 text-center rounded-tr-2xl rounded-br-2xl">
        <button
          ...
          onClick={() => handleDelete(interview.objectId)}
        >
